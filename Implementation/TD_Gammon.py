			...
			for k in range(self.num_output):
				for j in range(self.num_hidden):
					self.hidden_eligibility_trace[j][k] = (self.lamb * self.hidden_eligibility_trace[j][k] +
						self.gradient(self.hidden_weights[j][k]) * self.hidden_layer[j] * self.output_layer[k])#self.output_layer[side])
			...
			for j in range(self.num_hidden):
					for i in range(self.num_inputs):
						self.input_eligibility_trace[i][j] = (self.lamb * self.input_eligibility_trace[i][j] + 
							self.gradient(self.input_weights[i][j]) * self.input_layer[i] * self.hidden_layer[j])#self.output_layer[side])#
