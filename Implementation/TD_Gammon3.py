...
class TDGammon:
	def __init__(self, num_inputs, num_hidden, num_output, lr=0.01, d=0.95, l=0):
			...
			for k in range(self.num_output):
				...
				for j in range(self.num_hidden):
					...
					self.hidden_eligibility_trace[j][k] = (self.lamb *
						...
						self.gradient(self.output_layer[k]))
					self.hidden_weights[j][k] += self.learning_rate * (reward + self.discount_rate * self.output_layer[k] -
						self.preoutput_layer[k])* self.hidden_eligibility_trace[j][k]
