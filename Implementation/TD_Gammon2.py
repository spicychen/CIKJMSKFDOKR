...
class TDGammon:
	...
	def TDlearn(self, reward, side):
		...
		if self.preoutput_layer is not None:
			...
			hid_max = max(list(enumerate(self.hidden_layer)), key=lambda t:t[1])[0:1]
			#hid_max_pos = [i for i,j in enumerate(self.hidden_layer) if j==in_max]
			
			out_max = max(list(enumerate(self.hidden_layer)), key=lambda t:t[1])[0:1]
			#out_max_pos = [i for i,j in enumerate(self.output_layer) if j==out_max]
